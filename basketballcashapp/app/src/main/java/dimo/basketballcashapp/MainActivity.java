...
public class MainActivity extends AppCompatActivity {
    ...
    public  void  LoadAd(View view){
        this.rewardedAd=new RewardedAd(this,"ca-app-pub-4362411284535315/9137875495");
...
//                b.setBackgroundResource(R.drawable.wrong_button);
    ...
    private void InsertingInfo() {
        ...
        if(randomNum==0){
            findViewById(R.id.background).setBackgroundResource(R.drawable.finish_curry);
        }
        if(randomNum==1){
            findViewById(R.id.background).setBackgroundResource(R.drawable.finish_gianis);
        }
        if(randomNum==2){
            findViewById(R.id.background).setBackgroundResource(R.drawable.finish_harden);
        }
        if(randomNum==3){
            findViewById(R.id.background).setBackgroundResource(R.drawable.finish_kobe);
        }
        if(randomNum==4){
            findViewById(R.id.background).setBackgroundResource(R.drawable.finish_lebron);
        }
        ...
        for (User ur:listOfUsers) {
            tempListOfUsers.add(ur);
        }
        for(User ur:tempListOfUsers){
            if(ur.Username.equals(LogedInUser)){
                SecondUser=ur;
                MyDeleteUser(ur);
            }
        }
        SecondUser.NumOfPlays++;
        mUsersReference.push().setValue(SecondUser);
        ...
        tv.setText("Congratulation!\nYou earned " + Score + " points in " + mTime + " seconds.");
        LoadAd(findViewById(R.id.WrongEmail));
        ShowAd(findViewById(R.id.WrongEmail));
    }

    private void MyDeleteUser(User strTitle) {
        Query deleteQuery = mUsersReference.orderByChild("Username").equalTo(strTitle.Username);
    ...
    public void SignUpUser(View view){
        ...
        final EditText em=findViewById(R.id.RegisterEmailEdittext);
        mTimer.schedule(new TimerTask() {
            @Override
            public void run() {
                runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        if(em.getText().toString().equals("")){
                            em.setBackgroundResource(R.drawable.my_edittext);
                            findViewById(R.id.WrongEmail).setVisibility(View.INVISIBLE);

                        }
                        else{
                            if(em.getText().toString().contains("@")){
                                if(em.getText().toString().contains(".com")){
                                    em.setBackgroundResource(R.drawable.correct_user_or_pass);
                                    findViewById(R.id.WrongEmail).setVisibility(View.INVISIBLE);
                                }
                                else {
                                    em.setBackgroundResource(R.drawable.wrong_user_or_pass);
    ...
    public void RegisterAccount(View view) {
        ...
        if (!em.getText().toString().contains("@") || !em.getText().toString().contains(".com")) {
            s += i + ". Wrong email address\n";
        }
        if (!s.equals(""))
            WrongRegistering(s);
        else {
            User mUser = new User(un.getText().toString(), em.getText().toString(), pw.getText().toString(),0);
